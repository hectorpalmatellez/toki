...
describe("parser", () => {
  it("parses a minimal color + dimension document into a flat token list", () => {
    ...
    expect(doc.tokens.map((t) => t.id)).toEqual([
      "color.primary",
      "color.secondary",
      "spacing.small",
    ]);
    ...
    expect(doc.tree.children["color"]?.kind === "group" && doc.tree.children["color"]?.$type).toBe(
      "color",
    );
  ...
  it("rejects a top-level bare token (document must be a group)", () => {
    expect(() => parseTokenDocument({ $value: "#fff", $type: "color" })).toThrow(
  ...
  it("rejects an invalid $type value", () => {
    expect(() =>
      parseTokenDocument({ color: { $type: "hex", red: { $value: "#f00" } } }),
