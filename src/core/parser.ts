...
const isReservedKey = (key: string): key is ReservedKey =>
...
const parseToken = (
  ...
  if (!isTokenValue($value)) {
    throw new ParseError(
      `Token at ${path.join(".")} has an unsupported $value (function or null).`,
    );
  }
...
const parseGroup = (
  ...
  for (const key of Object.keys(raw)) {
    ...
    if (!isPlainObject(child)) {
      ...
      throw new ParseError(
        `Group${where} expected a child object but found ${typeof child}.`,
      );
    }
