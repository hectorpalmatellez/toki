...
const token = (
  id: string,
  value: TokenValue,
  overrides: Partial<ResolvedToken> = {},
): ResolvedToken =>
  Object.freeze({
    path: id.split("."),
    id,
...
describe("diffTokens", () => {
  ...
  it("detects changed tokens", () => {
    ...
    const updated = new Map([
      ["color.primary", token("color.primary", "#00ff00")],
    ]);
    const result = diffTokens(old, updated);
    expect(result.added).toHaveLength(0);
    expect(result.removed).toHaveLength(0);
  ...
  it("treats equal objects as unchanged", () => {
    ...
    const old = new Map([
      ["typography.body", token("typography.body", value, { type: "typography" })],
    ]);
    const updated = new Map([
      ["typography.body", token("typography.body", { ...value }, { type: "typography" })],
    ]);
  ...
  it("detects changes in array values", () => {
    const old = new Map([
      ["shadow.default", token("shadow.default", [{ x: 0, y: 2 }], { type: "shadow" })],
    ]);
    const updated = new Map([
      ["shadow.default", token("shadow.default", [{ x: 0, y: 4 }], { type: "shadow" })],
    ]);
    const result = diffTokens(old, updated);
    expect(result.changed).toHaveLength(1);
  });
...
describe("formatDiffTerminal", () => {
  ...
  it("formats changed tokens with ~ prefix and arrow", () => {
    const result = diffTokens(
      new Map([["a", token("a", "#fff")]]),
      new Map([["a", token("a", "#000")]]),
    );
    const output = formatDiffTerminal(result);
