...
import { discoverConfig, loadConfigFile, validateConfig, loadConfig, mergeConfig, DEFAULT_NAMING } from './config.js';
...
describe('loadConfigFile', () => {
  it('loads a .ts config file via jiti', () => {
    ...
    writeFileSync(configPath, 'export default { input: "./tokens.json", output: "./dist" }');
  ...
  it('loads a .js config file via jiti', () => {
    ...
    writeFileSync(configPath, 'module.exports = { input: "./tokens.json", output: "./dist" }');
    const config = loadConfigFile(configPath) as Record<string, unknown>;
    expect(config['input']).toBe('./tokens.json');
    expect(config['output']).toBe('./dist');
  });
...
describe('validateConfig', () => {
  ...
  it('rejects invalid themes', () => {
    expect(() => validateConfig({ input: './tokens.json', output: './dist', themes: 'not-an-object' })).toThrow(
      /must be a plain object/,
    );
  });

  it('rejects invalid theme value', () => {
    expect(() => validateConfig({ input: './tokens.json', output: './dist', themes: { light: 42 } })).toThrow(
  ...
  it('rejects invalid naming convention', () => {
    expect(() => validateConfig({ input: './tokens.json', output: './dist', naming: { css: 'invalid' } })).toThrow(
      /must be one of/,
    );
  });
  ...
  it('rejects non-function transforms', () => {
    expect(() => validateConfig({ input: './tokens.json', output: './dist', transforms: ['not-a-fn'] })).toThrow(
      /must be a function/,
    );
  });

  it('rejects invalid clean type', () => {
    expect(() => validateConfig({ input: './tokens.json', output: './dist', clean: 'yes' })).toThrow(
      /must be a boolean/,
    );
  });
...
describe('mergeConfig', () => {
  ...
  it('uses config input when CLI has none', () => {
    const result = mergeConfig({ input: './config-tokens.json', output: './dist' }, {});
    expect(result.input).toBe('./config-tokens.json');
  });

  it('CLI input takes precedence over config', () => {
    const result = mergeConfig({ input: './config-tokens.json', output: './dist' }, { input: './cli-tokens.json' });
    expect(result.input).toBe('./cli-tokens.json');
  });
  ...
  it('uses config formats when CLI format is empty', () => {
    const result = mergeConfig({ input: './tokens.json', output: './dist', formats: ['css'] }, {});
    expect(result.formats).toEqual(['css']);
  });

  it('defaults to css and js when nothing specified', () => {
    const result = mergeConfig({ input: './tokens.json', output: './dist' }, {});
  ...
  it('uses config clean when set', () => {
    const result = mergeConfig({ input: './tokens.json', output: './dist', clean: false }, {});
