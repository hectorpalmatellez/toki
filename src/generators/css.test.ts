...
const generate = async (raw: unknown): Promise<string> => {
  const tokens = resolveDocument(parseTokenDocument(raw));
  const [artifact] = await cssGenerator.generate(tokens, { version: '0.1.0' });
...
describe('css generator', () => {
  it('emits a :root block with kebab-case custom properties', async () => {
  ...
  it('includes the generated-by header comment', async () => {
    const css = await generate({ color: { $type: 'color', primary: { $value: '#1a73e8' } } });
  ...
  it('is deterministic — identical input produces byte-identical output', async () => {
    ...
    expect(await generate(raw)).toBe(await generate(raw));
  });

  it('emits resolved reference values (not the raw {…} syntax)', async () => {
    const css = await generate({
      color: {
        $type: 'color',
        primary: { $value: '#1a73e8' },
  ...
  it('formats cubicBezier tokens as a CSS cubic-bezier() function', async () => {
    const css = await generate({
  ...
  it('formats single shadow objects and multi-shadow arrays', async () => {
    const css = await generate({
      shadow: {
        $type: 'shadow',
  ...
  it('expands typography tokens into longhand CSS custom properties', async () => {
    const css = await generate({
      type: {
        $type: 'typography',
  ...
  it('expands partial typography tokens (only defined fields)', async () => {
    const css = await generate({
      type: {
        $type: 'typography',
  ...
  it('expands border tokens into longhand CSS custom properties', async () => {
    const css = await generate({
      border: {
        $type: 'border',
  ...
  it('expands transition tokens into longhand CSS custom properties', async () => {
    const css = await generate({
      transition: {
        $type: 'transition',
  ...
  it('writes to the css/ subdirectory', async () => {
    const [artifact] = await cssGenerator.generate(
  ...
  it('emits a README.md artifact for the output subdirectory', async () => {
    const artifacts = await cssGenerator.generate(
      resolveDocument(parseTokenDocument({ color: { $type: 'color', a: { $value: '#fff' } } })),
      { version: '0.1.0' },
    );
  ...
  it('matches the tokens.css snapshot', async () => {
    const css = await generate({
      color: {
        $type: 'color',
        primary: { $value: '#1a73e8' },
